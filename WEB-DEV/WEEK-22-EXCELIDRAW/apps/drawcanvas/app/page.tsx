...
import { ArrowRight, Paintbrush, Share2, Users } from "lucide-react";
import Link from "next/link";

export default function LandingPage100xCanvas() {
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-[138deg] from-[#AEEAEC] via-[#FFC9D0] to-[#56C5FF]">
      <header className="w-full px-4 lg:px-6 h-16 flex items-center backdrop-blur-md bg-white/20">
        <Link href="/" className="flex items-center justify-center">
          <Paintbrush className="w-6 h-6 mr-2 text-white" />
          <span className="font-bold text-xl text-white">100x Canvas</span>
        </Link>
          ...
          <div className="container px-4 md:px-6">
            <div className="flex flex-col items-center space-y-8 text-center">
              ...
              <div className="space-x-4">
                <Button className="bg-white/95 shadow-lg backdrop-blur-sm hover:bg-white transition-all duration-300 transform hover:scale-105 hover:shadow-xl">
                  <Link href="/canvas" className="flex items-center justify-center gap-2 text-blue-600 font-semibold group-hover:text-blue-700 transition-colors  ">
                    Start Creating
                    <ArrowRight className="h-5 w-5 group-hover:translate-x-0.5 transition-transform" />
                  </Link>
                </Button>
                <Button className="bg-transparent border-2 border-slate-700 text-slate-700 hover:bg-white/20 transition-all duration-300 transform hover:scale-105">
                  Explore Gallery
                </Button>
              </div>
            </div>
